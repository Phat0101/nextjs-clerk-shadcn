...
export const updateCompilerStep = mutation({
  ...
  handler: async (ctx, { jobId, step, analysisResult, confirmedFields, extractedData, supplierName, templateFound }) => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) throw new Error("Not authenticated");
...
// Public action to save extracted data (used by server-side agent where auth is unavailable)
