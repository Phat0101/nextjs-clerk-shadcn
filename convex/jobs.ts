...
export const saveN10ExtractedData = action({
  ...
  handler: async (ctx, { jobId, data }) => {
    await ctx.runMutation(internal.jobs._setN10ExtractedData, {
      jobId,
      data,
    });
  },
...
export const saveN10ExtractedDataPartial = action({
  ...
  handler: async (ctx, { jobId, partial }) => {
    console.log('saveN10ExtractedDataPartial', { jobId, partial });
    await ctx.runMutation(internal.jobs._mergeN10ExtractedDataInternal, {
      jobId,
      partial,
    });
  },
...
export const _setN10ExtractedData = internalMutation({
  args: {
    jobId: v.id('jobs'),
    data: v.any(),
  },
